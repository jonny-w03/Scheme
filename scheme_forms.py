...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        try:
            if expressions.rest.first.first:
                env.define(expressions.first, scheme_eval(expressions.rest.first, env))
        except AttributeError:
            env.define(expressions.first, expressions.rest.first)
    ...
    try:
        ...
        if expressions.rest is nil:
            if is_scheme_true(current_val):
                return current_val
        elif is_scheme_true(current_val):
    ...
    try:
        ...
        if expressions.rest is nil:
            if is_scheme_false(current_val):
                return current_val
        elif is_scheme_false(current_val):
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            try:
                if len(clause.first) < 2 and clause.first != "else": 
                    return clause.first
            except:
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    # BEGIN PROBLEM 14
    def make_names_pair(bindings):
        validate_form(bindings.first, 2, 2)
        if bindings.rest is not nil:
            return Pair(bindings.first.first, make_names_pair(bindings.rest))
        return Pair(bindings.first.first, nil)
    def make_values_pair(bindings):
        try:
            test_value = int(bindings.first.rest.first)
            if bindings.rest is not nil:
                return Pair(bindings.first.rest.first, make_values_pair(bindings.rest))
            return Pair(bindings.first.rest.first, nil)
        except:
            if isinstance(bindings.first.rest, Pair):
                current_val = eval_all(bindings.first.rest, env)
            else:
                current_val = env.lookup(bindings.first.rest.first)
            if bindings.rest is not nil:
                return Pair(current_val, make_names_pair(bindings.rest))
            return Pair(current_val, nil)
    if bindings is not nil and bindings.rest is not nil:
        validate_form(bindings.first, 2, 2)
        names = Pair(bindings.first.first, make_names_pair(bindings.rest))
        values = Pair(eval_all(bindings.first.rest, env), make_values_pair(bindings.rest))
    elif bindings is not nil:
        validate_form(bindings.first, 2, 2)
